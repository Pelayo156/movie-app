...
import { faUser, faPlus, faBars, faTimes } from "@fortawesome/free-solid-svg-icons"; // Importa faBars y faTimes
...
function Navbar() {
  ...
  return (
    <nav className="flex px-4 md:px-10 lg:px-20 h-20 justify-between items-center bg-black shadow-2xl fixed top-0 left-0 right-0 z-50">
      <Link to="/" onClick={closeMobileMenu}> {/* Cierra el menú si se hace clic en el logo */}
        <img src={logoUrl} className="w-32 md:w-40" alt="Logo" /> {/* Ajuste de tamaño de logo para responsividad */}
      </Link>
      ...
      <div className="md:hidden flex items-center space-x-3">
        ...
        <button onClick={toggleMobileMenu} className="text-white text-2xl focus:outline-none">
          <FontAwesomeIcon icon={isMobileMenuOpen ? faTimes : faBars} /> {/* Cambia el ícono según el estado */}
        </button>
      </div>

      {/* Menú de Navegación Principal (oculto en pantallas pequeñas, visible en medianas y grandes) */}
      <div className="hidden md:flex flex-grow justify-center space-x-8 lg:space-x-14"> {/* flex-grow para centrar */}
      ...
      <div className="hidden md:flex space-x-3"> {/* Solo visible en md y mayores */}
        <FontAwesomeIcon
          icon={faPlus}
          className="text-white text-xl cursor-pointer"
        />
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-20 left-0 w-full bg-black flex flex-col items-center py-4 shadow-lg z-40">
          <MenuButton text="Películas" url="/movies" onClick={closeMobileMenu} />
